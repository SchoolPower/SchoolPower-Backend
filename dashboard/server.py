...
Width, Height, Curve = 700, 500, 0.1
...
def index():
    ...
    number_chart.add("", list(number_chart_data.keys()), list(number_chart_data.values()), is_label_show=True,
                     is_datazoom_show=True, line_curve=Curve)
    ...
    number_usage_chart.add("", list(number_usage_chart_data.keys()), list(number_usage_chart_data.values()),
                           is_label_show=True, is_datazoom_show=True, line_curve=Curve)

    today_user, today_usage = number_chart_data.get(today_date, 0), number_usage_chart_data.get(today_date, 0)
    diff_user, diff_usage = today_user - number_chart_data.get(yesterday_date,
    ...
    version_chart_data_set = datatoaster.DataSet(unique_data_array)
    version_chart_data_set_data, series = standardized(version_chart_data_set
                                                       .set_x(lambda i: i.date)
                                                       .set_y(
        version_chart_data_set.NumberOfAppearance(lambda i: "%s (%s)" % (i.version, i.os)))
                                                       .ordered_by(lambda i: i[0])
                                                       .get_result())
    series = list(series)
    version_chart = Line("Versions", width=Width, height=Height)
    for i in range(0, len(next(iter(version_chart_data_set_data.values())))):
        version_chart.add(series[i], list(version_chart_data_set_data.keys()),
                          [j[i] for j in list(version_chart_data_set_data.values())], is_label_show=True,
                          is_datazoom_show=True, line_curve=Curve)
